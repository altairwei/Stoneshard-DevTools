function scr_console_spawn()
{
    ...
    if (_argumentsArrayLength > 1)
    {
        ...
        switch _ai
        {
            ...
            case 0:
                ...
                break
            default:
                scr_console_output_list("ai argument must be boolean", red)
                return;
        }

    }

    var _unitName = _argumentsArray[0]
    var _unit = noone
    switch _unitName
    {
        case "bandit01":
    ...
    if _unit
    {
        with (scr_enemy_create(mouse_x, mouse_y, _unit, 0))
            ai_is_on = _ai
        scr_console_output_list(("Unit '" + _unitName + "' spawned"), green)
    }
    else
        scr_console_output_list(("Unit '" + _unitName + "' not found"), red)
}
